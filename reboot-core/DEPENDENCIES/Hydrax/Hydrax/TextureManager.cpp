...
namespace Hydrax
{
	...
	bool TextureManager::_updateNormalMap(Image &Image)
	{
        ...
        for (x = 0; x < ImageSize.Width; x++)
        {
            ...
            {
                *pDest++ =   static_cast<Ogre::uint8>(Image.getValue(x,y,2)); // B
                *pDest++ =   static_cast<Ogre::uint8>(Image.getValue(x,y,1)); // G
                *pDest++ =   static_cast<Ogre::uint8>(Image.getValue(x,y,0)); // R
				        *pDest++ =   255;                                             // A
            }
        }
