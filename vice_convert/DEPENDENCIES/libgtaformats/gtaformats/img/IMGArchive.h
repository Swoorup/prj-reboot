...
public:
	...
	istream* gotoEntry(string name, bool autoCloseStream = false)
	{
		istream* lol;
		char * writable = new char[name.size() + 1];
		std::copy(name.begin(), name.end(), writable);
		writable[name.size()] = '\0'; // don't forget the terminating 0
		
		lol = gotoEntry(writable, autoCloseStream);
	...
	EntryIterator getEntryByName(string name) { return getEntryByName(name.c_str()); }
