...
namespace Hydrax{namespace Noise
{
	...
	void Perlin::_calculeNoise()
	{
		...
		for(o=0; o<octaves; o++)
		{
			...
			amount[0] = static_cast<int>(scale_magnitude*f_multitable[o]*(pow(sin((fraction+2)*PI_3),2)/1.5));
			amount[1] = static_cast<int>(scale_magnitude*f_multitable[o]*(pow(sin((fraction+1)*PI_3),2)/1.5));
			amount[2] = static_cast<int>(scale_magnitude*f_multitable[o]*(pow(sin((fraction  )*PI_3),2)/1.5));
	...
	float Perlin::_getHeigthDual(float u, float v)
	{
		...
		int ui = static_cast<int>(u*magnitude),
		    vi = static_cast<int>(v*magnitude),
			i,
			value = 0,
