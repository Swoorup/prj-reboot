...
map<CString, boost::shared_ptr<IMGArchive> > IMGArchiveMap;
...
File::File(const CString& path)
		: path(new FilePath(path)), autoDeletePath(true),
		  archivePtr(boost::shared_ptr< boost::shared_ptr<IMGArchive> > (new boost::shared_ptr<IMGArchive>))
{
}
...
File::filesize File::getSize() const
{
	if (!physicallyExists()) {
		if (path->isIMGPath()) {
			...
			try {
				boost::shared_ptr<IMGArchive> img = getIMGArchive();
