...
CString FilePath::getFileTitle() const
{

	int len = this->getFileName().length();
	char* filename = new char[len];
	strncpy(filename, this->getFileName().get(), len + 1);

	char* parentEnd = strrchr(filename, '.');

	char* txdName = new char[parentEnd - filename + 1 + 3];
	strncpy(txdName, filename, parentEnd - filename);
	return CString::from(txdName);
}
